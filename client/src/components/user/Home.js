        ...
        {this.state.contests.map(contest=>{
            ...
            <div className='m-2 bg-light'>
                <div className='border-bottom text text-dark d-flex justify-content-between p-3'>
                    <Link to={`/contest/${contest._id}/announcement/get`}>{`${contest.name} #${contest.number}`}</Link><span>{String(new Date(new Date(contest.startsAt).getTime() + (5.5 * 60 * 60 * 1000)))}</span>
                </div>
